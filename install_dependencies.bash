...
# Apparently this doesn't get installed so install it after we jump in the shell
...
if [ ! $only_reproduce ]; then
  ...
  # Hack: polymer-bundler won't install normally, so install it elsewhere and copy it into the clusterfuzz setup
  venv=$(${PYTHON} -m pipenv --venv)
  workingd=$(pwd)
  cd /tmp
  npm install polymer-bundler
  cp -R /tmp/node_modules/* "$venv/lib/node_modules/"
  ln -s "../lib/node_modules/polymer-bundler/lib/bin/polymer-bundler.js" "$venv/bin/polymer-bundler"
  cd $workingd
